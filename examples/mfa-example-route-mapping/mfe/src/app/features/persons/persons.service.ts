...
export class PersonsService {
  ...
  addPerson(person: PersonRequest): Promise<PersonResponse> {
    return firstValueFrom(
      this.http.post<PersonResponse>('/v1/dutch/familyties', person, { headers: this.jsonHeaders })
    );
  }
  ...
  deletePerson(lastName: string, firstName: string): Promise<void> {
    return firstValueFrom(
      this.http.delete<void>(
        `/v1/dutch/familyties/lastnames/${encodeURIComponent(lastName)}?firstname=${encodeURIComponent(firstName)}`
      )
    );
  }
}
